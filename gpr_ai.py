...
class GPRInterface:
    ...
    def create_widgets(self):
        ...
        self.import_button = tk.Button(button_frame, text="Importar Imagem", command=self.import_image)
        self.import_button.pack(side=tk.LEFT, padx=5)

        self.analyze_button = tk.Button(button_frame, text="Analisar Imagem", command=self.analyze_image,
                                        state=tk.DISABLED)
        self.analyze_button.pack(side=tk.LEFT, padx=5)

        self.add_pattern_button = tk.Button(button_frame, text="Adicionar Padrão", command=self.add_pattern)
        self.add_pattern_button.pack(side=tk.LEFT, padx=5)

        self.image_label = tk.Label(self.main_frame)
        self.image_label.pack(pady=10)

        self.result_text = scrolledtext.ScrolledText(self.main_frame, height=10, width=50)
        self.result_text.pack(pady=10)

        # Aba de Padrões
        self.patterns_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.patterns_frame, text='Padrões')

        self.patterns_tree = ttk.Treeview(self.patterns_frame, columns=('ID', 'Nome', 'Descrição'), show='headings')
        self.patterns_tree.heading('ID', text='ID')
        self.patterns_tree.heading('Nome', text='Nome')
        self.patterns_tree.heading('Descrição', text='Descrição')
        self.patterns_tree.pack(pady=10, padx=10, expand=True, fill='both')

        patterns_button_frame = tk.Frame(self.patterns_frame)
        patterns_button_frame.pack(pady=10)

        self.edit_pattern_button = tk.Button(patterns_button_frame, text="Editar Padrão", command=self.edit_pattern)
        self.edit_pattern_button.pack(side=tk.LEFT, padx=5)

        self.delete_pattern_button = tk.Button(patterns_button_frame, text="Excluir Padrão",
                                               command=self.delete_pattern)
        self.delete_pattern_button.pack(side=tk.LEFT, padx=5)

        # Aba CNN
        self.cnn_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.cnn_frame, text='CNN')

        cnn_button_frame = tk.Frame(self.cnn_frame)
        cnn_button_frame.pack(pady=10)

        self.cnn_import_button = tk.Button(cnn_button_frame, text="Importar Imagem para CNN",
                                           command=self.cnn_import_image)
        self.cnn_import_button.pack(side=tk.LEFT, padx=5)

        self.cnn_predict_button = tk.Button(cnn_button_frame, text="Prever com CNN", command=self.cnn_predict,
                                            state=tk.DISABLED)
        self.cnn_predict_button.pack(side=tk.LEFT, padx=5)

        self.cnn_train_button = tk.Button(cnn_button_frame, text="Treinar CNN", command=self.cnn_train)
        self.cnn_train_button.pack(side=tk.LEFT, padx=5)

        self.cnn_save_button = tk.Button(cnn_button_frame, text="Salvar Modelo", command=self.save_cnn_model)
        self.cnn_save_button.pack(side=tk.LEFT, padx=5)

        self.cnn_load_button = tk.Button(cnn_button_frame, text="Carregar Modelo", command=self.load_cnn_model)
        self.cnn_load_button.pack(side=tk.LEFT, padx=5)

        self.cnn_evaluate_button = tk.Button(cnn_button_frame, text="Avaliar Modelo", command=self.evaluate_cnn_model)
        self.cnn_evaluate_button.pack(side=tk.LEFT, padx=5)

        self.cnn_image_label = tk.Label(self.cnn_frame)
        self.cnn_image_label.pack(pady=10)

        self.cnn_result_text = scrolledtext.ScrolledText(self.cnn_frame, height=10, width=50)
        self.cnn_result_text.pack(pady=10)

        # Nova aba para Add Images to Dataset
        self.add_images_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.add_images_frame, text='Add Images to Dataset')

        add_images_button_frame = tk.Frame(self.add_images_frame)
        add_images_button_frame.pack(pady=10)

        self.open_add_images_button = tk.Button(add_images_button_frame, text="Abrir Add Images to Dataset", command=self.open_add_images_to_dataset)
        self.open_add_images_button.pack(side=tk.LEFT, padx=5)

        self.select_dataset_button = tk.Button(add_images_button_frame, text="Selecionar Diretório do Dataset", command=self.select_dataset_dir)
        self.select_dataset_button.pack(side=tk.LEFT, padx=5)

        self.add_images_button = tk.Button(add_images_button_frame, text="Adicionar Imagens", command=self.add_images_to_dataset)
        self.add_images_button.pack(side=tk.LEFT, padx=5)

        self.add_images_result_text = scrolledtext.ScrolledText(self.add_images_frame, height=10, width=50)
        self.add_images_result_text.pack(pady=10)

        # Nova aba para Organizar Imagens
        self.organize_images_frame = ttk.Frame(self.notebook)
